...
def test_from_file():
    ...
    try:
        ...
        expected = {Variable("S"): Regex("(a | b) c | $"), }
...
@pytest.mark.parametrize(
    "text, expected",[
        ...
        S -> $
        C -> a b c
             """, { Variable("S"): Regex("$"), Variable("C"): Regex("a b c"),},),],
)
...
@pytest.mark.parametrize(
    "cfg, expected_cfg", [("""
                ...
                S -> epsilon
                H -> S
                """, {Variable("S"): Regex("S a S | a H a | $"),
                      Variable("H"): Regex("S")},), ],
)
def test_from_cfg(cfg, expected_cfg):
    ecfg = ECFG.from_cfg(CFG.from_text(cfg))
    ecfg_productions = ecfg.productions
    assert all(regex_to_dfa(p.body).is_equivalent_to(regex_to_dfa(expected_cfg[p.head])
                                                     )
